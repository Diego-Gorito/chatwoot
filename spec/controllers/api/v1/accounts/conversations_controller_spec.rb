...
RSpec.describe 'Conversations API', type: :request do
  ...
  describe 'POST /api/v1/accounts/{account.id}/conversations/:id/sync_group' do
    let(:agent) { create(:user, account: account, role: :agent) }
    let(:group_contact) { create(:contact, account: account, group_type: :group, identifier: 'group-123') }
    let(:conversation) { create(:conversation, account: account, contact: group_contact) }

    context 'when it is an unauthenticated user' do
      it 'returns unauthorized' do
        post "/api/v1/accounts/#{account.id}/conversations/#{conversation.display_id}/sync_group"
    ...
    context 'when it is an authenticated user' do
      it 'syncs the group successfully' do
        create(:inbox_member, user: agent, inbox: conversation.inbox)
      ...
      it 'returns bad request when contact is not a group' do
        ...
        individual_conversation = create(:conversation, account: account, contact: individual_contact)
        create(:inbox_member, user: agent, inbox: individual_conversation.inbox)

        post "/api/v1/accounts/#{account.id}/conversations/#{individual_conversation.display_id}/sync_group",
             headers: agent.create_new_auth_token,
             as: :json
      ...
      it 'returns internal server error when provider is unavailable' do
        create(:inbox_member, user: agent, inbox: conversation.inbox)
        allow_any_instance_of(Conversation).to receive(:sync_group).and_raise( # rubocop:disable RSpec/AnyInstance
